...
resource "kubernetes_deployment" "keycloak_radius" {
  metadata {
...
resource "kubernetes_service" "keycloak_http" {
  ...
  spec {
    selector = {
      app = kubernetes_deployment.keycloak_radius.spec[0].template[0].metadata[0].labels.app
    }
    port {
      port        = 8080
      target_port = 8080
...
resource "kubernetes_service" "keycloak_radius" {
  metadata {
    name      = "keycloak-radius"
    namespace = "keycloak"
  }
